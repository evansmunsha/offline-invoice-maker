...
async function initDigitalGoods() {
  ...
  try {
    ...
    console.error("Failed to init Digital Goods Service", err);
  }
}
...
async function purchasePremiumWithGooglePlay() {
  ...
  const productId = "premium_unlock"; // EXACT Play Console ID
  ...
  const paymentRequest = new PaymentRequest(
    ...
    { total: { label: "Premium", amount: { currency: "USD", value: "0.00" } } }
  );
  ...
  try {
    ...
    await digitalGoodsService.acknowledge(
      purchaseToken,
      "onetime"
    );
...
async function restorePurchases() {
  ...
  purchases.forEach(p => {
    if (p.itemId === "premium_unlock") {
      unlockPremium();
    }
  });
}

document.addEventListener("DOMContentLoaded", restorePurchases);
...
document.addEventListener("DOMContentLoaded", initDigitalGoods,() => {
  const purchaseBtn = document.getElementById("purchasePremium");

  if (!purchaseBtn) return;

  if (isAndroidPWA()) {
    purchaseBtn.onclick = purchaseWithGooglePlay;
  } else {
    purchaseBtn.onclick = () => {
      alert("Please install the Android app to purchase.");
    };
  }
});
